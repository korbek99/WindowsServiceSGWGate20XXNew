...
namespace NetClient
{
    public partial class Client : BaseForm
    {
            ...
            switch (_iAlarmType)
            {
                ...
                case AlarmConstMsgType.ALARM_VCA_INFO:
                    ...
                    //{
                        ...
                        //int ret;
                        //ret = NVSSDK.NetClient_GetComFormat_V1(_ulLogonID, 1,  cDeviceType,  cComFormat, 1);

                            TypeRulesBF objRule = new TypeRulesBF();
                            ...
                            int_rule=m_lpbuf.m_iRuleID;
                            dsRule = objRule.GetTypeRulesByIDBF(int_rule);

                            if (dsRule.Tables[0].Rows.Count > 0)
                            {
                                foreach (DataRow dataRow in dsRule.Tables[0].Rows)
                                {
                                    int_rule = Convert.ToInt32(dataRow["ID"].ToString());
                                    str_VCARules = dataRow["Nombre"].ToString();
                                }
                            }
        ...
        {
            EstadisticasNextiva ObjForm = new EstadisticasNextiva();
            ObjForm.Show();
        }
